  ...
  if (!process.env.NODE_ENV || process.env.NODE_ENV === 'development') {
    ...
    socket.on('get-numbers-homepage', (array) => {
      ...
      numberList.forEach(num => {
        if ((num >= 16 && num <=30) || (num >= 46 && num <=60)) document.getElementById(num.toString()).style.background = "rgb(45, 212, 191)"
    ...
    socket.on('send-number', number => {
      ...
      setTimeout(() => {
        document.getElementById("fadingDivChild").textContent = `${letter}-${number}`
    ...
    socket.on('remove-number', number => {
      ...
      document.getElementById("fadingDiv").classList.remove("fading-div")
      if ((number >= 16 && number <=30) || (number >= 46 && number <=60)) document.getElementById(number.toString()).style.background = "rgb(45, 212, 191)"
      else document.getElementById(number.toString()).style.background = '#007079'
      let newList = numberList
