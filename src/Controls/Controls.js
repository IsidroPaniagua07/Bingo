...
const Controls = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      parseInt(inputValue) > 75 ||
      parseInt(inputValue) < 1
    )
      return alert("Please use numbers between 1-75");
    const parsedNumber = parseInt(inputValue);
    if (calledNumbers.includes(parsedNumber))
      return alert("You've already selected that number");
    document.getElementById(
      "submitNotification"
    ).textContent = `Selected: ${parsedNumber}`;
  ...
  return (
    <>
      <div className="flex flex-col h-full w-full justify-center items-center bg-jcaqua">
        <div
          className="flex flex-row bg-gray-200 h-full w-full
        ...
        >
          ...
          <div className="flex flex-col justify-center items-center h-full w-full pt-6 pr-2">
            <div className="flex flex-col h-full w-full items-center text-center">
              <div className="h-[80%] pt-2 text-5xl">Bingo Controls</div>
              <div className="flex h-full text-4xl items-center">
                Ball Count: {ballCount}
              </div>
              <div className="flex flex-col justify-between h-full w-full items-center">
                <button
                  onClick={sendBingoEmit}
                  className="h-auto w-auto text-lg md:text-xl lg:text-2xl bg-jcaqua text-white p-1 m-1 font-semibold outline-black outline outline-2"
                >
                  Call Bingo
                </button>
                <button
                  onClick={sendClearEmit}
                  className="h-auto w-auto text-lg md:text-xl lg:text-2xl text-white bg-red-600 p-1 m-1 font-semibold outline-black outline outline-2"
                >
                  Clear Board
                </button>
              </div>
            ...
            >
              <div
                ...
                className="h-fit w-full text-center text-2xl lg:text-3xl justify-self-center self-center"
              >
                Type number below!
              </div>
              <input
                ref={inputRef}
                autoComplete="off"
                className="text-black text-center text-5xl lg:text-6xl h-1/6 w-2/3 max-w-[324px] outline outline-1"
                type="text"
                name="name"
              />
              <button
                type="submit"
                className="h-[10vh] w-2/3 max-w-[324px]  p-1 bg-green-700 font-bold text-3xl md:text-4xl lg:text-5xl text-white mt-20 outline-black outline outline-2"
              >
                Submit
              </button>
            </form>
